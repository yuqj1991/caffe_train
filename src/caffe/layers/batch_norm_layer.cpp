...
void BatchNormLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    if (bottom[0] != top[0]) {
        ...
        //caffe_copy(top[0]->count(), top[0]->cpu_diff(), top[0]->mutable_cpu_diff()); // top[0]->x_norm
        caffe_set(top[0]->count(), Dtype(0.), top[0]->mutable_cpu_diff()); // new_added
        top_diff = top[0]->cpu_diff(); // top[0]->x_norm
    }
    Dtype* bottom_diff = bottom[0]->mutable_cpu_diff();
    int num = bottom[0]->shape()[0];
    int spatial_dim = bottom[0]->count()/(bottom[0]->shape(0)*channels_);
    const Dtype* top_data = top[0]->cpu_data(); // top[0]->x_norm
    const Dtype* var_data = variance_.cpu_data();
    if (use_global_stats_) {
