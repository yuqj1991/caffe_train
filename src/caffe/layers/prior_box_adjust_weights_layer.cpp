...
#include <vector>
#include <math.h>
#include "caffe/layers/prior_box_adjust_weights_layer.hpp"

namespace caffe {

template <typename Dtype>
void PriorBoxWeightsLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void PriorBoxWeightsLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void PriorBoxWeightsLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const int layer_width = bottom[0]->width();
  const int layer_height = bottom[0]->height();
