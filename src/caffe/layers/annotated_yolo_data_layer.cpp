...
#include "caffe/layers/annotated_yolo_data_layer.hpp"
...
YoloAnnotatedDataLayer<Dtype>::YoloAnnotatedDataLayer(const LayerParameter& param)
  : BasePrefetchingDataLayer<Dtype>(param),
    reader_(param) {
}

template <typename Dtype>
YoloAnnotatedDataLayer<Dtype>::~YoloAnnotatedDataLayer() {
  this->StopInternalThread();
}

template <typename Dtype>
void YoloAnnotatedDataLayer<Dtype>::DataLayerSetUp(
  ...
  if (transform_param.resize_param_yolo_size()) {
    if (transform_param.resize_param_yolo(0).resize_mode() ==
  ...
  vector<int> top_shape =
      this->data_transformer_->InferBlobShape_yolo(anno_datum.datum(),0);
...
template<typename Dtype>
void YoloAnnotatedDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  AnnotatedDatum& anno_datum = *(reader_.full().peek());
  // Use data_transformer to infer the expected blob shape from anno_datum.
  int num_resize_policies = transform_param.resize_param_yolo_size();
  int policy_num = 0;
  if (num_resize_policies > 0) {
	  std::vector<float> probabilities;
	  float prob_sum = 0;
	  for (int i = 0; i < num_resize_policies; i++) {
		  const float prob = transform_param.resize_param_yolo(i).prob();
		  CHECK_GE(prob, 0);
		  CHECK_LE(prob, 1);
		  prob_sum += prob;
  ...
  vector<int> top_shape =
      this->data_transformer_->InferBlobShape_yolo(anno_datum.datum(), policy_num);
  this->transformed_data_.Reshape(top_shape);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    vector<int> shape =
        this->data_transformer_->InferBlobShape_yolo(sampled_datum->datum(), policy_num);

	//LOG(INFO) << shape[2] << "," << shape[3];
    if (transform_param.resize_param_yolo_size()) {
      if (transform_param.resize_param_yolo(policy_num).resize_mode() ==
          ResizeParameter_Resize_mode_FIT_SMALL_SIZE) {
    ...
    if (this->output_labels_) {
      if (has_anno_type_) {
        ...
        this->data_transformer_->Transform_yolo(*sampled_datum,
...
INSTANTIATE_CLASS(YoloAnnotatedDataLayer);
REGISTER_LAYER_CLASS(YoloAnnotatedData);
