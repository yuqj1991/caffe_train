...
void DetectionEvaluateLayer<Dtype>::LayerSetUp(
  ...
  attri_type_ = detection_evaluate_param.attri_type();
...
void DetectionEvaluateLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if(attri_type_ == DetectionEvaluateParameter_AnnoataionAttriType_FACE){
...
void DetectionEvaluateLayer<Dtype>::Forward_cpu(
  ...
  GetDetectionResults(det_data, bottom[0]->height(), background_label_id_,
                    &all_detections, attri_type_);
  /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
  if(attri_type_ == DetectionEvaluateParameter_AnnoataionAttriType_FACE){
    // Retrieve all ground truth (including difficult ones).
    map<int, LabelBBox> all_gt_bboxes;
    AnnotatedDatum_AnnoataionAttriType annotype = AnnotatedDatum_AnnoataionAttriType_FACE;
    ...
    for (int c = 0; c < num_classes_; ++c) {
      ...
      top_data[num_det * 7] = -1;
      top_data[num_det * 7 + 1] = c;
      if (num_pos.find(c) == num_pos.end()) {
        top_data[num_det * 7 + 2] = 0;
      } else {
        top_data[num_det * 7 + 2] = num_pos.find(c)->second;
      }
      top_data[num_det * 7 + 3] = -1;
      top_data[num_det * 7 + 4] = -1;
      top_data[num_det * 7 + 5] = -1;
      top_data[num_det * 7 + 6] = -1;
      ++num_det;
    }
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
      ...
      if (all_gt_bboxes.find(image_id) == all_gt_bboxes.end()) {
        ...
        for (LabelBBox::iterator iit = detections.begin();
          ...
          for (int i = 0; i < bboxes.size(); ++i) {
            top_data[num_det * 7] = image_id;
            top_data[num_det * 7 + 1] = label;
            top_data[num_det * 7 + 2] = bboxes[i].score();
            top_data[num_det * 7 + 3] = 0;
            top_data[num_det * 7 + 4] = 1;
            top_data[num_det * 7 + 5] = 0;
            top_data[num_det * 7 + 6] = 0;
            ++num_det;
          }
        }
      }else{
              ...
              if (overlap_max >= overlap_threshold_) {
                if (evaluate_difficult_gt_ ||
                  ...
                  if (!visited[jmax]) {
                    // true positive.
                    top_data[num_det * 7 + 3] = 1;
                    top_data[num_det * 7 + 4] = 0;
                    if(gt_bboxes[jmax].faceattrib().blur() == bboxes[i].faceattrib().blur()){
                      top_data[num_det * 7 + 5] = 1;
                    }else
                    {
                      top_data[num_det * 7 + 5] = 0;
                    }
                    if(gt_bboxes[jmax].faceattrib().occlusion() == bboxes[i].faceattrib().occlusion()){
                      top_data[num_det * 7 + 6] = 1;
                    }else
                    {
                      top_data[num_det * 7 + 6] = 0;
