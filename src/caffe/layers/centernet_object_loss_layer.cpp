...
void CenterObjectLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (num_gt_ >= 1) {
    ...
    EncodeCenteGroundTruthAndPredictions(loc_data, output_width, output_height, 
                                share_location_, loc_pred_data, num_channels,
                                loc_gt_data, all_gt_bboxes);
  ...
  if(iterations_ % 1000 == 0){
    ...
    LOG(INFO)<<"num_classes: "<<num_classes_<<", output_width: "<<output_width<<", output_height: "<<output_height;
  }
  iterations_++;
  #endif
}
