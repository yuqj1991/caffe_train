...
#include "caffe/layers/face_contour_data_layer.hpp"
#include "caffe/util/benchmark.hpp"

namespace caffe{

template <typename Dtype>
faceContourDataLayer<Dtype>::faceContourDataLayer(const LayerParameter & param)
    : BasePrefetchingDataLayer<Dtype>(param),
    reader_(param){
}

template <typename Dtype>
faceContourDataLayer<Dtype>::~faceContourDataLayer(){
    this->StopInternalThread();
}

template <typename Dtype>
void faceContourDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
    ...
    AnnoFaceContourDatum& anno_datum = *(reader_.full().peek());
    ...
    if (this->output_labels_) {
        ...
        if (has_anno_type_) {
            ...
            if (anno_type_ == AnnoFaceContourDatum_AnnoType_FACECONTOUR) {
...
template<typename Dtype>
void faceContourDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
    ...
    for (int item_id = 0; item_id < batch_size; ++item_id) {
        timer.Start();
        // get a anno_datum
        AnnoFaceContourDatum& anno_datum = *(reader_.full().pop("Waiting for data"));
        #if 0
    ...
    #endif
        AnnoFaceContourDatum distort_datum;
        AnnoFaceContourDatum* expand_datum = NULL;
        if (transform_param.has_distort_param()) {
            distort_datum.CopyFrom(anno_datum);
            this->data_transformer_->DistortImage(anno_datum.datum(),
                                                    distort_datum.mutable_datum());
            if (transform_param.has_expand_param()) {
                expand_datum = new AnnoFaceContourDatum();
        ...
        reader_.free().push(const_cast<AnnoFaceContourDatum*>(&anno_datum));
    }
    ...
    if (this->output_labels_ && has_anno_type_) {
        ...
        if (anno_type_ == AnnoFaceContourDatum_AnnoType_FACECONTOUR) {
            ...
            for (int item_id = 0; item_id < batch_size; ++item_id) {
                ...
                top_label[idx++] = face.point_3().x();
                top_label[idx++] = face.point_4().x();
                top_label[idx++] = face.point_5().x();
                top_label[idx++] = face.point_6().x();
                top_label[idx++] = face.point_7().x();
                top_label[idx++] = face.point_8().x();
                top_label[idx++] = face.point_9().x();
                top_label[idx++] = face.point_10().x();
                top_label[idx++] = face.point_11().x();
                top_label[idx++] = face.point_12().x();
                top_label[idx++] = face.point_13().x();
                top_label[idx++] = face.point_14().x();
                top_label[idx++] = face.point_15().x();
                top_label[idx++] = face.point_16().x();
                top_label[idx++] = face.point_17().x();
                top_label[idx++] = face.point_18().x();
                top_label[idx++] = face.point_19().x();
                top_label[idx++] = face.point_20().x();
                top_label[idx++] = face.point_21().x();
                top_label[idx++] = face.point_1().y();
                top_label[idx++] = face.point_2().y();
                top_label[idx++] = face.point_3().y();
                top_label[idx++] = face.point_4().y();
                top_label[idx++] = face.point_5().y();
                top_label[idx++] = face.point_6().y();
                top_label[idx++] = face.point_7().y();
                top_label[idx++] = face.point_8().y();
                top_label[idx++] = face.point_9().y();
                top_label[idx++] = face.point_10().y();
                top_label[idx++] = face.point_11().y();
                top_label[idx++] = face.point_12().y();
                top_label[idx++] = face.point_13().y();
                top_label[idx++] = face.point_14().y();
                top_label[idx++] = face.point_15().y();
                top_label[idx++] = face.point_16().y();
                top_label[idx++] = face.point_17().y();
                top_label[idx++] = face.point_18().y();
                top_label[idx++] = face.point_19().y();
                top_label[idx++] = face.point_20().y();
                top_label[idx++] = face.point_21().y();
            }
        } else {
            LOG(FATAL) << "Unknown annotation type.";
        }
...
INSTANTIATE_CLASS(faceContourDataLayer);
REGISTER_LAYER_CLASS(faceContourData);
