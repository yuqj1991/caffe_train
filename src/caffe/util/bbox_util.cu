  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (code_type == PriorBoxParameter_CodeType_CORNER) {
      ...
      if (variance_encoded_in_target) {
        // variance is encoded in target, we simply need to retore the offset
        // predictions.
        decode_bbox_xmin = exp(xmin) * prior_width + p_xmin;
        decode_bbox_ymin = exp(ymin) * prior_height + p_ymin;
        decode_bbox_xmax = exp(xmax) * prior_width + p_xmax;
        decode_bbox_ymax = exp(ymax) * prior_height + p_ymax;
      } else {
        // variance is encoded in bbox, we need to scale the offset accordingly.
        decode_bbox_xmin =
        exp(prior_data[vi] * xmin) * prior_width + p_xmin;
          decode_bbox_ymin =
          exp(prior_data[vi + 1] * ymin) * prior_height + p_ymin;
          decode_bbox_xmax =
          exp(prior_data[vi + 2] * xmax) * prior_width + p_xmax;
          decode_bbox_ymax =
          exp(prior_data[vi + 3] * ymax) * prior_height + p_ymax;
      }
