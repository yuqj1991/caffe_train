...
__global__ void DecodeBBoxesKernel(const int nthreads,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (code_type == PriorBoxParameter_CodeType_CORNER) {
      ...
      Dtype bbox_xmin, bbox_xmax;
      Dtype bbox_ymin, bbox_ymax;
      if (variance_encoded_in_target) {
        // variance is encoded in target, we simply need to retore the offset
        // predictions.
        bbox_xmin = -xmin * prior_width + prior_center_x;
        bbox_ymin = -ymin * prior_height + prior_center_y;
        bbox_xmax = -xmax * prior_width + prior_center_x;
        bbox_ymax = -ymax * prior_height + prior_center_y;
      } else {
        // variance is encoded in bbox, we need to scale the offset accordingly.
        bbox_xmin = -prior_data[vi] * xmin * prior_width + prior_center_x;
        bbox_ymin = -prior_data[vi + 1] * ymin * prior_height + prior_center_y;
        bbox_xmax = -prior_data[vi + 2] * xmax * prior_width + prior_center_x;
        bbox_ymax = -prior_data[vi + 3] * ymax * prior_height + prior_center_y;
      }
      switch (i) {
        case 0:
          bbox_data[index] = bbox_xmin;
          break;
        case 1:
          bbox_data[index] = bbox_ymin;
          break;
        case 2:
          bbox_data[index] = bbox_xmax;
          break;
        case 3:
          bbox_data[index] = bbox_ymax;
          break;
      }
      ...
      switch (i) {
        ...
        case 1:
          ...
          break;
        case 2:
          bbox_data[index] = bbox_xmin + decode_bbox_width;
          break;
        case 3:
          bbox_data[index] = bbox_ymin + decode_bbox_height;
          break;
      }
    } else {
      // Unknown code type.
