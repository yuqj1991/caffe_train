    ...
    for block in blocks:
        ...
        if block['type'] == 'net':
            ...
            if len(layers) == 1:
                ...
                assert(prev_width == out_widths[layers[1]])
                assert(prev_height == out_heights[layers[1]])
                assert(out_widths[layers[2]] == out_widths[layers[1]])
                assert(out_heights[layers[2]] == out_heights[layers[1]])
                ...
                assert(out_widths[layers[2]] == out_widths[layers[1]])
                assert(out_heights[layers[2]] == out_heights[layers[1]])
                assert(out_widths[layers[2]] == out_widths[layers[3]])
                assert(out_heights[layers[2]] == out_heights[layers[3]])
            ...
            width = prev_width * stride
            height = prev_height * stride
            print('%5d %-6s           /%2dx  %3d x %3d x%4d   ->   %3d x %3d x%4d' % (ind, 'upsample', stride, prev_width, prev_height, prev_filters, width, height, filters))
            prev_width = width
            prev_height = height
            prev_filters = filters
            out_widths.append(prev_width)
