...
def static_histogram_data_from_all_sample(source_folder,label_folder):
	...
	for sub_folder in sub_image_folder:
		...
		for image_file in file_list:
			...
			xml_file = xml.dom.minidom.parse('../wider_face/Annotations/'+image_file.split('.')[-2]+'.xml')
...
def draw_histogram_base_data():
	static_data = static_histogram_data_from_all_sample('../wider_face/JPEGImages/wider_train/images', '../wider_face/label')
...
def main():
	...
	for file in wider_directory:
		shuffle_file('../wider_face/ImageSets/Main'+'/'+file+'.txt')
