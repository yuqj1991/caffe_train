        ...
        for mode = 0:1
            ...
            for i = trainIdx
                ...
                for p = 1:numPeople
                    ...
                    for part = 1:17
                        joint_all(count).joint_self(part, 1) = anno(part*3-2); %part的x坐标
                        joint_all(count).joint_self(part, 2) = anno(part*3-1); %part的y坐标

                        if(anno(part*3) == 2)
                            joint_all(count).joint_self(part, 3) = 1; %存在关节点标记
                        elseif(anno(part*3) == 1)
                            joint_all(count).joint_self(part, 3) = 0;
                        else
                            joint_all(count).joint_self(part, 3) = 2; %不存在关节点标记
                        end
                    end

                    % pad it into 17x3
                    dim_1 = size(joint_all(count).joint_self, 1); %返回行数
                    dim_3 = size(joint_all(count).joint_self, 3); %若为彩色图像，返回3；若为灰度图像，返回1；这里返回1
                    pad_dim = 17 - dim_1;  %这里可能是为了防止丢失某些keypoints而做的填充
                    joint_all(count).joint_self = [joint_all(count).joint_self; zeros(pad_dim, 3, dim_3)]; %(pad_dim, 3, dim_3)维的零元素
                    % set scale
