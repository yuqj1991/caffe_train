...
CAFFE_ROOT = '/home/resideo/workspace/caffe'
...
def main():
	...
	while True:
		...
		for m in range(len(multiplier)-1):
			...
			pose_net.blobs['data'].reshape(*(1, 3, imageToTest_padded.shape[0], imageToTest_padded.shape[1]))
			# net.forward() # dry run
			pose_net.blobs['data'].data[...] = np.transpose(np.float32(imageToTest_padded[:, :, :, np.newaxis]), (3, 2, 0, 1)) / 256 - 0.5;
			start_time = time.time()
			output_blobs = pose_net.forward()
			print('At scale %d, The CNN took %.2f ms.' % (m, 1000 * (time.time() - start_time)))
			# extract outputs, resize, and remove padding
			heatmap = np.transpose(np.squeeze(pose_net.blobs[output_blobs.keys()[1]].data), (1, 2, 0))  # output 1 is heatmaps
			...
			paf = np.transpose(np.squeeze(pose_net.blobs[output_blobs.keys()[0]].data), (1, 2, 0))  # output 0 is PAFs
				...
				for i in range(nA):
					for j in range(nB):
						...
						score_with_dist_prior = sum(score_midpts) / len(score_midpts) + min(
							0.5 * frame.shape[0] / (norm + 0.000045)- 1, 0)
